...
```{r, echo=FALSE}
we_major <-  get_tidy_audio_analysis("46fk9wjYcPm0sgym2b7EEE") %>% # Change URI.
...
```{r, echo = FALSE}
we_major %>%  compmus_self_similarity(pitches, "manhattan") %>% 
  ggplot(
    aes(
...
```
```{r, echo = FALSE}
we_major %>%  compmus_self_similarity(timbre, "manhattan") %>% 
  ggplot(
    aes(
      x = xstart + xduration / 2,
      width = xduration,
